...
public class Sharpen extends Filter {
    ...
    */
    private Sharpen(String name, double[][] filter) {
        this.name = name;
        filterMap.put(name, filter);
    }

    public static Sharpen getInstance(String name, double[][] filter) {
        if (s == null) {
            s = new Sharpen(name, filter);
        }
        return s;
    }
